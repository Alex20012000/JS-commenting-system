...
class App {
  ...
  usedElements: HTMLElements = {
    ...
    commentsCounter: document.querySelector(
      ".comments-counter"
    ) as HTMLSpanElement,
  ...
  handleSubmit(e: SubmitEvent) {
    ...
    this.usedElements.commentsCounter.innerHTML = `
          <span class="comments-superscript">Комментарии</span> ${
            !this.commentID ? "(0)" : `(${this.commentID})`
          }
        `;
    ...
    for (const comment of sorted) {
      ...
      if (comment.parent) {
        ...
        const newCommentDiv = document.querySelector(
          `.comment[data-id="${comment.parent.id}"]`
        ) as HTMLDivElement;
        const parent = newCommentDiv.closest(
          ".newCommentDiv"
        ) as HTMLDivElement;
...
class Commentary {
  ...
  setNewTemplate(isReply: boolean = false) {
    const date = this.timestamp.toLocaleString("ru-RU", {
      day: "2-digit",
      month: "2-digit",
    });
    const time = this.timestamp.toLocaleString("ru-RU", {
      hour: "2-digit",
      minute: "2-digit",
    });
    ...
    return `
              <div class="newCommentDiv">
                 <div class="comment ${isReply ? "active" : ""}" data-id="${
      this.id
    }">
                      <img src="${
                        this.author.avatar
  ...
  getNewHTML(isReplyComment = false) {
    ...
    const replyButton = this.newComment.querySelector(
      ".respond"
    ) as HTMLSpanElement;
  ...
  handleCommentReply() {
    ...
    const input = replyInput.elements.namedItem(
      "replyInput"
  ...
  handleAddToFavorite() {
    const favText = this.newComment!.querySelector(
      ".addToFav span"
    ) as HTMLSpanElement;
    const heartImg = this.newComment!.querySelector(
      ".addToFav img"
    ) as HTMLImageElement;
    ...
    this.isFavorite
      ...
      : (heartImg.src = "./images/likeHeart-not-filled.svg");
  ...
  handleLikeClicks(e: MouseEvent) {
    ...
    const likesCounter = this.newComment!.querySelector(
      ".initial"
    ) as HTMLDivElement;
